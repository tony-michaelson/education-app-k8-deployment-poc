...
class AuthService @Inject()(config: Configuration) {
  ...
  def getIdTokenClaims(idToken: String): Try[JwtClaim] =
    for {
      jwk    <- getJwk(idToken)                                                    // Get the secret key for this token
      claims <- JwtJson.decode(idToken, jwk.getPublicKey, Seq(JwtAlgorithm.RS256)) // Decode the token using the secret key
    } yield claims
