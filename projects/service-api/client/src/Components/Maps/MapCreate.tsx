...
> {
  ...
  createMap() {
    ...
    window.mpio.createMap(
      ...
      (newMapID) => {
        if (this.state.image) {
          window.mpio.setMapIcon(newMapID, this.state.image, () => {
            this.exit();
          });
        } else {
          this.exit();
  ...
  handleFileSelection() {
    return (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (file) {
        ...
        this.setState({ thumbnail: undefined, image: undefined });
      }
    };
  }
  ...
  @autobind
  exit() {
    location.href = '/maps?' + Math.floor(Math.random() * 10000000000);
  }
    ...
    if (this.state.loading === true) {
      ...
      return (
        <Modal
          ...
          size="lg"
        >
          <ModalHeader toggle={this.toggle}>Create New Course</ModalHeader>
          ...
          <ModalFooter>
            ...
            {!this.state.saving && (
              <Button color="primary" onClick={this.createMap}>
                Create
              </Button>
            )}
