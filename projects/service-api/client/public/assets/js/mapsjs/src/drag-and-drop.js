...
MAPJS.dragdrop = function (mapModel, stage) {
  ...
  var currentDroppable,
    ...
    isPointOverNode = function (x, y, node) {
    ...
    tryFlip = function (rootNode, nodeBeingDragged, nodeDragEndX) {
      var flipRightToLeft =
          rootNode.x < nodeBeingDragged.x && nodeDragEndX < rootNode.x,
        flipLeftToRight =
          rootNode.x > nodeBeingDragged.x && rootNode.x < nodeDragEndX;
    ...
    nodeDragEnd = function (
      ...
      var nodeBeingDragged = mapModel.getCurrentLayout().nodes[id],
        ...
        currentRank = parentIdea.findChildRankById(id),
        maxSequence = 1,
        validReposition = function () {
          return (
            nodeBeingDragged.level === 2 ||
      ...
      // Move Node and Update Rank
      idea.positionBefore(id, verticallyClosestNode.id);
      var newRank = parentIdea.findChildRankById(id);

      // Update Rank in DB
      var updateRankInDB = function (id, order) {
        mpio.patchMapNode(
          {
            order: order,
          },
      ...
      if (currentRank !== newRank) {
        if (typeof parentIdea.findSubIdeaById(id).attr.id != 'undefined') {
          updateRankInDB(parentIdea.findSubIdeaById(id).attr.id, newRank);
        }
      }
      ...
      if (shouldPositionAbsolutely && validReposition()) {
        ...
        maxSequence = _.max(
          _.map(parentIdea.ideas, function (i) {
            return (
              (i.id !== id &&
                i.attr &&
                i.attr.position &&
  ...
  mapModel.addEventListener('nodeCreated', function (n) {
    ...
    node.on('dragmove', function (evt) {
      ...
      nodeDragMove(n.id, stagePoint.x, stagePoint.y);
    });
