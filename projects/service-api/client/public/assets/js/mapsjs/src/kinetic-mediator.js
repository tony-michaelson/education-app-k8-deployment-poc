...
Kinetic.Stage.prototype.isRectVisible = function (rect, offset) {
  ...
  rect = rect
    .xscale(scale)
    .xtranslate(offset.x, offset.y)
    .xinset(offset.margin);
  return !(
...
MAPJS.KineticMediator = function (mapModel, stage) {
  ...
  var layer = new Kinetic.Layer(),
    ...
    moveStage = function (deltaX, deltaY) {
      ...
      visibleBeforeMove =
        atLeastOneVisible(nodeByIdeaId, 0, 0) ||
        atLeastOneVisible(connectorByFromIdeaIdToIdeaId, 0, 0);
      visibleAfterMove =
        atLeastOneVisible(nodeByIdeaId, deltaX, deltaY) ||
        atLeastOneVisible(connectorByFromIdeaIdToIdeaId, deltaX, deltaY);
    ...
    ensureSelectedNodeVisible = function (node) {
      var scale = stage.getScale().x || 1,
        offset = 0, // set to 0 to avoid movement that interferes with dblclick issue #182
        move = { x: 0, y: 0 };
      if (!node.getIsSelected()) {
        return;
      }
      if (
        node.getAbsolutePosition().x + node.getWidth() * scale + offset >
        stage.getWidth()
      ) {
        move.x =
          stage.getWidth() -
      ...
      if (
        node.getAbsolutePosition().y + node.getHeight() * scale + offset >
        stage.getHeight()
      ) {
        move.y =
          stage.getHeight() -
  ...
  mapModel.addEventListener(
    ...
    function (n) {
      ...
      node.on('dblclick', function (evt) {
        if (evt.which == 1) {
          mapModel.dblClickEvent('mouse', n.id);
          return;
        }
      });
  ...
  mapModel.addEventListener('nodeDroppableChanged', function (
    ideaId,
    isDroppable,
  ) {
    var node = nodeByIdeaId[ideaId];
  ...
  mapModel.addEventListener('nodeRemoved', function (n) {
    ...
    node.off(
      'click dblclick tap dbltap dragstart dragmove dragend mouseover mouseout touchstart touchend :openAttachmentRequested :editing :textChanged ',
    );
    //	node.destroy();
    new Kinetic.Tween({
      node: node,
  ...
  mapModel.addEventListener('mapScaleChanged', function (
    scaleMultiplier,
    zoomPoint,
  ) {
    ...
    new Kinetic.Tween({
      ...
      x:
        zoomPoint.x +
        ((stage.getX() - zoomPoint.x) * targetScale) / currentScale,
      y:
        zoomPoint.y +
        ((stage.getY() - zoomPoint.y) * targetScale) / currentScale,
      scaleX: targetScale,
      scaleY: targetScale,
      easing: Kinetic.Easings.EaseInOut,
      duration: 0.01,
  ...
  mapModel.addEventListener('activatedNodesChanged', function (
    activatedNodes,
    deactivatedNodes,
  ) {
  ...
  function (content) {
    ...
    var iconSize =
      (content.attr &&
        content.attr.icon &&
        ':' +
          content.attr.icon.width +
          'x' +
          content.attr.icon.height +
          'x' +
          content.attr.icon.position) ||
      ':0x0x0';
