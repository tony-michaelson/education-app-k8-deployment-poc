...
MAPJS.content = function (contentAggregate, sessionKey) {
  ...
  var cachedId,
    ...
    init = function (contentIdea, originSession) {
      ...
      contentIdea.containsDirectChild = contentIdea.findChildRankById = function (
        childIdeaId,
      ) {
        return parseFloat(
          _.reduce(
      ...
      contentIdea.find = function (predicate) {
        var current = predicate(contentIdea)
          ? [_.pick(contentIdea, 'id', 'title')]
          : [];
      ...
      contentIdea.sortedSubIdeas = function () {
        ...
        var result = [],
          childKeys = _.groupBy(
            _.map(_.keys(contentIdea.ideas), parseFloat),
            function (key) {
              return key > 0;
            },
          ),
          sortedChildKeys = _.sortBy(childKeys[true], Math.abs).concat(
            _.sortBy(childKeys[false], Math.abs),
          );
    ...
    findIdeaById = function (ideaId) {
      return contentAggregate.id == ideaId
        ? contentAggregate
        : contentAggregate.findSubIdeaById(ideaId);
    },
    sameSideSiblingRanks = function (parentIdea, ideaRank) {
      return _(_.map(_.keys(parentIdea.ideas), parseFloat)).reject(function (
        k,
      ) {
        return k * ideaRank < 0;
      });
    ...
    appendChange = function (method, args, undofunc, originSession) {
      ...
      if (
        method === 'batch' ||
        batches[originSession] ||
        !eventStacks ||
        !eventStacks[originSession] ||
        eventStacks[originSession].length === 0
      ) {
        ...
        if (prev.eventMethod === 'batch') {
          ...
          eventStacks[originSession].push({
            eventMethod: 'batch',
            eventArgs: [[prev.eventMethod].concat(prev.eventArgs)].concat([
              [method].concat(args),
            ]),
            undoFunction: function () {
              undofunc();
      ...
      if (isRedoInProgress) {
        contentAggregate.dispatchEvent(
          'changed',
          'redo',
          undefined,
          originSession,
  ...
  contentAggregate.sameSideSiblingIds = function (subIdeaId) {
    ...
    return _.without(
      _.map(
        _.pick(parentIdea.ideas, sameSideSiblingRanks(parentIdea, currentRank)),
        function (i) {
          return i.id;
        },
      ),
  ...
  contentAggregate.clone = function (subIdeaId) {
    var toClone =
      (subIdeaId &&
        subIdeaId != contentAggregate.id &&
        contentAggregate.findSubIdeaById(subIdeaId)) ||
      contentAggregate;
  ...
  contentAggregate.calculatePath = function (
    ideaId,
    currentPath,
    potentialParent,
  ) {
    ...
    return _.reduce(
      ...
      function (result, child) {
        return (
          result ||
          contentAggregate.calculatePath(
            ideaId,
            [child].concat(currentPath),
            child,
          )
        );
      },
  ...
  contentAggregate.getSubTreeIds = function (rootIdeaId) {
    ...
    collectIds(
      contentAggregate.findSubIdeaById(rootIdeaId) || contentAggregate,
    );
    return result;
  };
  ...
  contentAggregate.endBatch = function (originSession) {
    ...
    if (_.size(inBatch) === 1) {
      logChange(
        inBatch[0].eventMethod,
        inBatch[0].eventArgs,
        inBatch[0].undoFunction,
        activeSession,
      );
    } else {
  ...
  contentAggregate.execCommand = function (cmd, args, originSession) {
    ...
    return commandProcessors[cmd].apply(
      contentAggregate,
      [originSession || sessionKey].concat(_.toArray(args)),
    );
  };
  ...
  commandProcessors.paste = function (
    originSession,
    parentIdeaId,
    jsonToPaste,
    initialId,
    ...
    var pasteParent =
        parentIdeaId == contentAggregate.id
          ? contentAggregate
          : contentAggregate.findSubIdeaById(parentIdeaId),
      cleanUp = function (json) {
        var result = _.omit(json, 'ideas', 'id'),
          index = 1,
          childKeys,
          sortedChildKeys;
        if (json.ideas) {
          childKeys = _.groupBy(
            _.map(_.keys(json.ideas), parseFloat),
            function (key) {
              return key > 0;
            },
          );
    ...
    newIdea =
      jsonToPaste &&
      (jsonToPaste.title || jsonToPaste.attr) &&
      init(cleanUp(jsonToPaste), sessionFromId(initialId));
    ...
    logChange(
      'paste',
      [parentIdeaId, jsonToPaste, newIdea.id],
      function () {
  ...
  commandProcessors.initialiseTitle = function (
    originSession,
    ideaId,
    title,
    newUUID,
    ...
    idea.title = title;
    idea.attr.nodeType = nodeType;
    if (newUUID) {
      idea.attr.id = newUUID;
      idea.attr.top_map_id = topMapUUID;
      idea.attr.path = path;
    }
    appendChange(
      'initialiseTitle',
  ...
  commandProcessors.updateTitle = function (originSession, ideaId, title) {
    ...
    parent = contentAggregate.findParent(ideaId) || idea;
  ...
  // ** Create New Node ** //
  commandProcessors.addSubIdea = function (
    originSession,
    parentId,
    ideaTitle,
    optionalNewId,
    ...
    var is_allowed;
    $.each(assoc_rules[parent.attr.nodeType], function (i, e) {
      if (e == global_node_type) {
        is_allowed = 'yes';
        return;
      }
    });
    ...
    if (is_allowed) {
      ...
      idea = init({
        ...
        id: optionalNewId,
        attr: {
          node_type: global_node_type,
          style: { background: node_type_colors[global_node_type] },
        },
      });
      ...
      switch (global_node_type) {
        case 'category':
          top.bootbox.alert(
            'Categories can only be added under other categories.',
          );
          break;
        case 'flashcard':
          top.bootbox.alert('Flashcards can only be added under a category.');
          break;

        default:
          top.bootbox.alert(
            'You cannot add this node type to parent type: ' +
              parent.attr.nodeType,
          );
      }
    }
  };
  ...
  commandProcessors.removeSubIdea = function (originSession, subIdeaId) {
    ...
    if (parent) {
      ...
      contentAggregate.links = _.reject(contentAggregate.links, function (
        link,
      ) {
      ...
      if (typeof oldIdea.attr.id != 'undefined') {
        mpio.deleteMapNode(
          top.deck_id,
          top.segmentID,
          oldIdea.attr.id,
  ...
  commandProcessors.insertIntermediate = function (
    originSession,
    inFrontOfIdeaId,
    title,
    optionalNewId,
    ...
    logChange(
      'insertIntermediate',
      [inFrontOfIdeaId, title, newIdea.id],
      function () {
  ...
  commandProcessors.changeParent = function (
    originSession,
    ideaId,
    newParentId,
  ) {
    ...
    if (
      typeof idea.attr.id != 'undefined' &&
      typeof contentAggregate.findParent(ideaId).attr.id != 'undefined'
    ) {
      mpio.patchMapNode(
        {
          order: newRank,
          parentID: contentAggregate.findParent(ideaId).attr.id,
        },
        top.deck_id,
        top.segmentID,
        idea.attr.id,
  ...
  var updateAttr = function (object, attrName, attrValue) {
    ...
    if (
      !attrValue ||
      attrValue === 'false' ||
      (_.isObject(attrValue) && _.isEmpty(attrValue))
    ) {
  ...
  contentAggregate.mergeAttrProperty = function (
    ideaId,
    attrName,
    attrPropertyName,
    attrPropertyValue,
  ...
  commandProcessors.updateAttr = function (
    originSession,
    ideaId,
    attrName,
    attrValue,
  ...
  contentAggregate.moveRelative = function (ideaId, relativeMovement) {
    var parentIdea = contentAggregate.findParent(ideaId),
      ...
      siblingRanks =
        currentRank &&
        _.sortBy(sameSideSiblingRanks(parentIdea, currentRank), Math.abs),
      ...
      newIndex =
        currentIndex +
        (relativeMovement > 0 ? relativeMovement + 1 : relativeMovement),
      beforeSibling =
        newIndex >= 0 &&
        parentIdea &&
        siblingRanks &&
        parentIdea.ideas[siblingRanks[newIndex]];
    if (newIndex < 0 || !parentIdea) {
      return false;
    }
    return contentAggregate.positionBefore(
      ideaId,
      beforeSibling && beforeSibling.id,
      parentIdea,
    );
  };
  ...
  commandProcessors.positionBefore = function (
    originSession,
    ideaId,
    positionBeforeIdeaId,
    parentIdea,
    ...
    if (!currentRank) {
      return _.reduce(
        ...
        function (result, idea) {
          return (
            result ||
            commandProcessors.positionBefore(
              originSession,
              ideaId,
              positionBeforeIdeaId,
              idea,
    ...
    if (positionBeforeIdeaId) {
      ...
      candidateSiblings = _.reject(_.sortBy(siblingRanks, Math.abs), function (
        k,
      ) {
        return Math.abs(k) >= Math.abs(afterRank);
      });
      beforeRank =
        candidateSiblings.length > 0 ? _.max(candidateSiblings, Math.abs) : 0;
    ...
    // issue #160; adding safeguard to ensure very long floats are truncated to 10 decimal places
    if (newRank.toString().length > 12) {
      newRank = newRank.toFixed(10);
    }
  ...
  (function () {
    ...
    commandProcessors.addLink = function (originSession, ideaIdFrom, ideaIdTo) {
      ...
      alreadyExists = _.find(contentAggregate.links, function (link) {
        return (
          (link.ideaIdFrom === ideaIdFrom && link.ideaIdTo === ideaIdTo) ||
          (link.ideaIdFrom === ideaIdTo && link.ideaIdTo === ideaIdFrom)
        );
      });
      if (alreadyExists) {
        return false;
    ...
    commandProcessors.removeLink = function (
      originSession,
      ideaIdOne,
      ideaIdTwo,
    ) {
      ...
      while (contentAggregate.links && i < contentAggregate.links.length) {
        ...
        if (
          String(link.ideaIdFrom) === String(ideaIdOne) &&
          String(link.ideaIdTo) === String(ideaIdTwo)
        ) {
    ...
    commandProcessors.updateLinkAttr = function (
      originSession,
      ideaIdFrom,
      ideaIdTo,
      attrName,
      ...
      if (undoAction) {
        logChange(
          'updateLinkAttr',
          [ideaIdFrom, ideaIdTo, attrName, attrValue],
          undoAction,
          originSession,
        );
      }
  ...
  commandProcessors.redo = function (originSession) {
    ...
    if (topEvent) {
      ...
      contentAggregate.execCommand(
        topEvent.eventMethod,
        topEvent.eventArgs,
        originSession,
      );
      isRedoInProgress = false;
